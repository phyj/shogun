...
CWDSVMOcas::CWDSVMOcas(
	float64_t C, index_t d, index_t from_d, CStringFeatures<uint8_t>* traindat,
	CLabels* trainlab)
...
SGVector<float64_t> CWDSVMOcas::apply_get_outputs(CFeatures* data)
{
	...
	if (features)
	{
		index_t num=features->get_num_vectors();
		ASSERT(num>0)
...
void* CWDSVMOcas::add_new_cut_helper( void* ptr)
{
	...
	for (index_t j=start; j<end; j++)
	{
		...
		index_t lim=CMath::min(degree, string_length-j);
		index_t len;

		for (index_t k=0; k<lim; k++)
		{
			bool free_vec;
